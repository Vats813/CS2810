...
public:
    ...
    //Getters for isbn, copies available and total copies.
    string getisbn(){
        return isbn; 
    }
    int getCA(){
    ...
    //Update copies function. Checks for negative values.
    ...
    //Borrow function. Checks for available copies.
    bool borrowBook(){
    ...
    //Return function. Ensures no extra returns.
    bool returnBook(){
    ...
    void printDetails(){
        cout<<title<<' '<<author<<endl; //Print function.
    }
};
class Member{
    string memberID;
...
public:
    ...
    //Default constructor. Error was showing, hence had to make one.
    Member(){
    }
    //Getter for ID.
    string getID(){
    ...
    //Borrow function. Checks for borrowlimit.
    ...
    //Return function. Checks if book is borrowed.
...
class Library{
    ...
    set<string>isbns; //Sets to easily check if an element is present/Duplicated.
    set<string>IDs;
public:
    //Add function. Checks for duplicates.
    bool addBook(Book &a){
        if(books.size()==50){
            return false;
        }
    ...
    //Updatecopies function, uses public member of Book class itself.
    ...
    //Register a new member function. Checks for duplicates. 
    ...
    //Borrow function. Uses borrow from both book and member class. Checks if valid for book then member.
    ...
    //Return function. Uses return from both book and member class. Checks if valid for book then member.
...
int main(){
    Library Lib;
    string op;
    while(true){ //Taking continuous input until Done string appears.
        cin>>op;
        if(op == "Done"){ 
            break;
        }else if(op == "Book"){ //Further inputs are taken based on the first string of the current line.
            string s1, s2, s3;
            int CA, TC;
            cin>>s1;
            if(s1 == "ExistingBook"){
